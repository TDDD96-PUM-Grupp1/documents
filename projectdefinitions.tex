...
\newcommand{\definition}[1]
{
  \begin{switch}{#1}
    ...
    \case{Instans}{\item [\textbf{#1}]
      En spelsession som startas från UI-applikationen och spelare kan gå med i för att spela spelet.}
    \case{IoT-backend}{\item [\textbf{#1}]
      Existerande system som kan dirigera data mellan uppkopplade enheter.}
    \case{Kontroll-applikation}{\item [\textbf{#1}]
      Applikation som körs på en mobil eller surfplatta och skickar styrningsdata till UI-applikationen.}
    ...
    Ett gyroskop som sitter på en mobil enhet. Används för att hämta data om mobila enhetens position och lutning.}
    ...
    \case{Scrum-bräde}{\item [\textbf{#1}]
      En tavla med lappar som innehåller aktiviteter som ska göras under
    ...
    \case{Burndown chart}{\item [\textbf{#1}]
      En graf som visar hur många timmar som återstår i projektet.}
    ...
    \case{Cybercom}{\item [\textbf{#1}]
      Kortare variant av Cybercom Group, företaget produkten utvecklas åt.}
    ...
    \case{Projektet}{\item [\textbf{#1}]
        Processen att framställa en produkt åt Cybercom Group. Denna produkt är ett spel som skickar sensordata från kontroll-applikationen till UI-applikationen via Cybercoms IoT-backend.}
    \case{Sensordata}{\item [\textbf{#1}]
      Datan från en accelerometer som beskriver hur mobilen accelererar gentemot marken. Denna kan användas med avseende på gravitationskraften för att beskriva vilken riktning mobilen befinner sig i.}
    \case{Software Quality Asssurance}{\item [\textbf{#1}]
    	Förkortat SQA, är en samling aktiviteter som bedömmer lämpligheten och anger förtroende
    ...
    \case{Teamet, gruppen}{\item [\textbf{#1}]
      Det team av åtta studenter som tillsammans har utfört projektet.}
    \case{Trello}{\item [\textbf{#1}]
      En hemsida för att lägga till och fördela uppgifter bland flera personer, kan liknas vid en tavla som lappar fästs på.}
    ...
    Ett begrepp som beskriver den tekniska och samhälleliga utveckling då fler och fler saker blir uppkopplade mot internet.}
    \case{Git-repo}{\item [\textbf{#1}]
    Ett arkiv för att versionshantera olika version av kod på Git.}
    \case{Master-gren}{\item [\textbf{#1}]
    Standardgrenen till ett git-repo som vanligtvis reflekterar repot i ett fungerande tillstånd.}
	\case{Kursen}{\item [\textbf{#1}]
    Den kurs som detta projekt utförs inom, det vill säga kursen ''Kandidatprojekt i programvaruutveckling'' med kurskod TDDD96 på Linköpings universitet.}
  \case{npm, Node Package Manager}{\item [\textbf{#1}]
    En pakethanterare för Javascripts ekosystem.}
  \case{npm-paket}{\item [\textbf{#1}]
    Ett paket med Javascript-kod som finns tillgängligt i npm.}
  \case{open-source}{\item [\textbf{#1}]
  Projekt som tillåter vem som helst att ladda ner, modifiera och tillföra förslag av ändringar till en existerande kodbas.}

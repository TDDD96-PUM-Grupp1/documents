...
För att analysera beroenden till paket i npm behövs först en robust beskrivning av hur dessa beroenden fungerar ges. Här presenteras hur npm implementerar beroenden samt den grafmodell som använts vid analysen. För att kunna svara på frågeställning \ref{joel_o-fs:3} definieras även de relevanta kvalitetsfaktorerna.
...
I de fall ett paket specifierar flera accepterade versioner i ett beroende har i denna undersökning endast den senaste versionen använts. Detta är konsekvent med npms beteende då paket hämtas. Det anses därför ointressant att behandla detta som flera olika beroenden.
